...
<body>
  ...
  const fnBoardList = ()=>{
	  $.ajax({
		  ...
		  /* 응답 */
		  dataType: 'json'
	  }).done((resData)=>{             // 스코프를 빠져나와서 success 가 아닌 done으로 해보는 버전
	    ...
	    $.each(resData, (i, board)=>{
	    	...
	    	// data-board-no 이거 js 할 때 배웠음 데이터속성에 카멜케이스 쓰지말고 이렇게 쓰라고! 상단부분에 설명해놨었음. 아 위치가 기억나지말고 내용이 기억나라고 ㅠ
	    })
	    result += '</div>';
   ...
   <script>
    ...
    const fnBoardDetail = ()=>{
      ...
      $(document).on('click', '.board', (evt)=>{
        const boardNo = evt.currentTarget.dataset.boardNo;  // $(evt.currentTarget).data('boardNo') 제이쿼리로 한다면 이렇게
        $.ajax({
          /* 요청 */
          type: 'GET',
